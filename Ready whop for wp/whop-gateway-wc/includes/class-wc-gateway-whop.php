...
class WC_Gateway_Whop extends WC_Payment_Gateway {
	...
	public function init_form_fields(): void {
		...
		$this->form_fields = [
			...
			'license_key' => [
				'title'       => __( 'License key (optional)', 'whop-gateway-wc' ),
				'type'        => 'text',
				'description' => __( 'Optional. From your purchase email — used for support identification only.', 'whop-gateway-wc' ),
				'default'     => '',
			],
			'support_url' => [
	...
	private function validate_saved_settings(): array {
		...
		if ( ! Whop_GW_Helper::has_credentials() ) {
			$errors[] = __( 'Whop Checkout is enabled but not configured. Open Whop Checkout → Setup Wizard.', 'whop-gateway-wc' );
			return $errors;
		}

		if ( '' === trim( Whop_GW_Helper::get_bearer_token() ) ) {
	...
	public function admin_options(): void {
		echo '<p><a class="button button-primary" href="' . esc_url( Whop_GW_Helper::admin_page_url( 'wizard' ) ) . '">'
			. esc_html__( 'Open Setup Wizard', 'whop-gateway-wc' ) . '</a></p>';
	...
	public function ajax_test_connection(): void {
		...
		if ( ! Whop_GW_Helper::get_bearer_token() ) {
			wp_send_json_error( [ 'message' => __( 'Enter an API key in the Setup Wizard first.', 'whop-gateway-wc' ) ] );
		}
