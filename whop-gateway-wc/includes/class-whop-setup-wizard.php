...
class Whop_GW_Setup_Wizard {
	...
	private static function render_step_credentials(): void {
		...
		if ( Whop_GW_Helper::is_connected() && 'oauth' === $settings['connection_mode'] ) {
			?>
			<div class="whop-gw-card">
				<h2><?php esc_html_e( 'Step 2 — Already connected', 'whop-gateway-wc' ); ?></h2>
				<p><?php esc_html_e( 'Your store is connected via Whop OAuth. You can skip to verification.', 'whop-gateway-wc' ); ?></p>
				<a class="button button-primary" href="<?php echo esc_url( Whop_GW_Helper::admin_page_url( 'wizard' ) . '&step=4' ); ?>">
					<?php esc_html_e( 'Skip to verification', 'whop-gateway-wc' ); ?>
				</a>
			</div>
			<?php
			return;
